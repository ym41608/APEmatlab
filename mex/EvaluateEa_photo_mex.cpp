...
void mexFunction(int nlhs, mxArray *plhs[],
    ...
    configs = mxGetPr(prhs[6]);
    xs = (int*)mxGetPr(prhs[7]);
    ys = (int*)mxGetPr(prhs[8]);
   // usePhotometrics = mxGetScalar(prhs[9]);
    ...
    for (i = 0 ; i < numConfigs ; i++) {       
        ...
        //if (!usePhotometrics) {    // main code(not use photometrics)
        //    for (int j = 0; j < numPoints ; j++) {
		//		double marker_x = double(*ptrXsc);
		//		double marker_y = double(*ptrYsc);
		//		//double marker_x = double(*ptrXsc) / w1;
		//		//double marker_y = - double(*ptrYsc) / h1;
...
//                                score += fabs( (xs_target[j]-meanX) - sigXoversigY*(ys_target[j]-meanY));
    ...
    //mexPrintf("Average Time elapsed %lf\n", (end-start) / numConfigs);
